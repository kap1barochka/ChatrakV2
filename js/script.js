...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (submitBtn) {
        submitBtn.addEventListener('click', () => {
            ...
            if (name === "" || text === "") {
                alert(window.chatrakT ? window.chatrakT("Please fill in all fields!") : "Please fill in all fields!");
                return;
            }
            ...
            const reviewHTML = `
                <div class="swiper-slide box">
                    ...
                    <p>${text}</p>
                    <h3>${name}</h3><span>${window.chatrakT ? window.chatrakT("new client") : "new client"}</span>
                </div>
            `;
            ...
            currentRating = 5;
            updateStars(currentRating);
            alert(window.chatrakT ? window.chatrakT("Thank you for your review!") : "Thank you for your review!");
        });
    }

    if (clearBtn) {
        clearBtn.addEventListener('click', () => {
            if(confirm(window.chatrakT ? window.chatrakT("Are you sure you want to delete all your reviews?") : "Are you sure you want to delete all your reviews?")) {
...
// Инициализация Firebase. Если CDN Firebase временно не загрузился, сайт не ломается:
// модальное окно, переключатель языков и кнопка закрытия продолжают работать.
let auth;
if (typeof firebase !== 'undefined' && firebase.apps && firebase.auth) {
    ...
    console.warn('Firebase is not available. Auth actions are temporarily disabled.');
    auth = {
        currentUser: null,
        signOut: () => Promise.resolve(),
        onAuthStateChanged: (callback) => { if (typeof callback === 'function') callback(null); },
        signInWithEmailAndPassword: () => Promise.reject(new Error('Firebase is not available.')),
        createUserWithEmailAndPassword: () => Promise.reject(new Error('Firebase is not available.'))
    };
}
...
function authT(text) {
    return window.chatrakT ? window.chatrakT(text) : text;
}

function refreshAuthTexts() {
    if (modalTitle) modalTitle.innerText = authT(isLoginMode ? "Sign In" : "Sign Up");
    if (modalSubtitle) modalSubtitle.innerText = authT(isLoginMode ? "Welcome back, chess master!" : "Join the ultimate chess community!");
    if (submitBtn) submitBtn.innerText = authT(isLoginMode ? "Login" : "Register");
    if (toggleText) toggleText.innerText = authT(isLoginMode ? "Don't have an account?" : "Already have an account?");
    if (toggleLink) toggleLink.innerText = authT(isLoginMode ? "Sign Up" : "Sign In");

    if (nameGroup) nameGroup.style.display = isLoginMode ? "none" : "block";
    if (nameInput) {
        nameInput.placeholder = authT("Your Full Name");
        if (isLoginMode) nameInput.removeAttribute('required');
        else nameInput.setAttribute('required', 'true');
    }
    if (emailInput) emailInput.placeholder = authT("Email Address");
    if (passwordInput) passwordInput.placeholder = authT("Password");
    if (closeBtn) closeBtn.setAttribute('aria-label', authT('Close'));
}
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && modal && modal.style.display === 'flex') closeAuthModal();
});
    ...
    authForm.addEventListener('submit', (e) => {
        ...
        if (isLoginMode) {
            auth.signInWithEmailAndPassword(email, password)
                ...
                .catch((error) => {
                    errorMsg.innerText = window.chatrakT ? window.chatrakT("Error: Invalid email or password.") : "Error: Invalid email or password.";
            ...
            auth.createUserWithEmailAndPassword(email, password)
                ...
                .then(() => {
                    modal.style.display = 'none';
                    authForm.reset();
                    alert(window.chatrakT ? window.chatrakT("Account created successfully!") : "Account created successfully!");
                })
                .catch((error) => {
...
auth.onAuthStateChanged((user) => {
    if (loginBtn) {
        if (user) {
            ...
            loginBtn.innerHTML = `<i class="fas fa-sign-out-alt"></i> ${authT("Exit")}`;
            ...
            loginBtn.innerHTML = authT("Sign In");
...
document.addEventListener('chatrak:languagechange', () => {
    refreshAuthTexts();
    if (typeof auth !== 'undefined' && auth && loginBtn) {
        if (auth.currentUser) loginBtn.innerHTML = `<i class="fas fa-sign-out-alt"></i> ${authT("Exit")}`;
        else loginBtn.innerHTML = authT("Sign In");
    }
});
