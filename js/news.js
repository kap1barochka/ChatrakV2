document.addEventListener('DOMContentLoaded', () => {
    ...
    function resolveArticleKey(title) {
        const normalized = String(title || '').trim();
        return Object.keys(articleTexts).find(key => {
            if (key === normalized) return true;
            if (window.ChatrakI18n && window.ChatrakI18n.t) {
                return window.ChatrakI18n.t(key, 'ru') === normalized || window.ChatrakI18n.t(key, 'hy') === normalized;
            }
            return false;
        }) || normalized;
    }
    ...
    readLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            const sourceTitle = resolveArticleKey(title);
            ...
            modalTitle.innerText = tr(sourceTitle);
            modalImg.src = imgSrc;
            modalCategory.innerText = tr(category);
            modalMeta.innerText = tr(meta);
            
            // Подставляем полный текст статьи
            modalBody.innerHTML = tr(articleTexts[sourceTitle] || "Full article text will be available soon.");
